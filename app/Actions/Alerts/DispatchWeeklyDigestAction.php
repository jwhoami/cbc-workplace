...
class DispatchWeeklyDigestAction
{
    use AsAction;
    use AsJob;
    ...
    public function handle(?Carbon $now = null): array
    {
        ...
        $windowKey = 'weekly:'.$now->format('o-W');
        ...
        JobAlert::query()
            ->active()
            ->withFrequency(JobAlertFrequency::Weekly)
            ->ofActiveMember()
            ->cursor()
            ->each(function (JobAlert $alert) use (&$counts, $windowStart, $windowEnd, $windowKey) {
                ...
                try {
                    ...
                    Util::logChange(
                        "alerts:dispatch-weekly failed for alert {$alert->id}",
                        'error',
                        'ALERT_DISPATCH',
                        ['error' => $e->getMessage()]
                    );
        ...
        Util::getActivityLog('job-alert.dispatch.weekly')
            ->withProperties(['summary' => $counts])
            ->log('Despacho de alertas semanales');
