...
class MemberPolicy extends BasePolicy
{
    ...
    public function requestMembership(Model $user, Member $member): bool
    {
        ...
        if (! in_array($member->membership_state, $validStates)) {
            return false;
        }
    ...
    public function approveMembershipRequest(Model $user, Member $member): bool
    {
        ...
        if (! $user->hasPermission(static::prefix('approve-membership'))) {
            return false;
        }
        if ($member->membership_state !== MembershipState::PENDING) {
            return false;
        }
